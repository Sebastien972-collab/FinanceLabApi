...
struct ArticleContentController: RouteCollection {
    func boot(routes: any RoutesBuilder) throws {
        ...
        protectedRoutes.get(":articleContentID", use: getById)
        protectedRoutes.delete(":articleContentID", use: delete)
        protectedRoutes.put(":articleContentID", use: update)
        protectedRoutes.get(use: getByArticleID)
    }

    func index(req: Request) async throws -> [ArticleContent] {
        
        try await ArticleContent.query(on: req.db).all()
    }
    ...
    func getById(req: Request) async throws -> ArticleContent {
        let payload = try req.auth.require(UserPayload.self)
...
//        articleContent.$user.id = try user.requireID()
        try await articleContent.save(on: req.db)
    ...
    func getByArticleID(req: Request) async throws -> [ArticleContent] {
        let payload = try req.auth.require(UserPayload.self)
